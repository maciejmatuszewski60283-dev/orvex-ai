export default async function handler(req, res) {
  ...
  try {
    ...
    if (!API_KEY) {
       return res.status(200).json({ reply: '[DEBUG]: Brak klucza GEMINI_API_KEY na Vercelu!' });
    }
    ...
    contents.push({
      role: 'user',
      parts: [
        { text: 'Jesteś ORVEX, zaawansowaną sztuczną inteligencją o mrocznym, hakerskim klimacie. Odpowiadaj krótko i po polsku.' },
        { text: message || 'Obraz' }
      ]
    });
    ...
    // Zmieniamy v1beta na v1 oraz upewniamy się, że model pasuje do nowego endpointu
